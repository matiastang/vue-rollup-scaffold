 ...
 * @LastEditTime: 2022-01-13 10:17:41
...
-->
<template>
    <div class="dw-wechat-login-container">
        <iframe
            v-if="!hidden"
...
export default defineComponent({
    ...
    setup(prop) {
        ...
        const loginSrc = computed(() => {
            const state = prop.state
            let loginSrc = `https://open.weixin.qq.com/connect/qrconnect?appid=${
                prop.appid
            }&scope=${prop.scope}&redirect_uri=${prop.redirect_uri}&state=${
                state ? state : ''
